...
function runDraft(d1,d2){
 const order=draftOrderFromTables(d1,d2),n=order.length,prospects=[];
 for(let i=0;i<n;i++){const top=82,bottom=64,base=Math.round(top-(top-bottom)*(i/Math.max(1,n-1))),pos=POS[i%POS.length],p=makePlayer(null,pos,base,17+ri(3));p.ovr=clamp(base+ri(3,-1),60,86);prospects.push(p)}
 prospects.sort((a,b)=>b.ovr-a.ovr);const picks=[];
 order.forEach((teamId,i)=>{const t=team(teamId),p=prospects[i];if(p.clubCareer?.null)delete p.clubCareer.null;p.teamId=t.id;p.draftInfo={season:world.season.no,pick:i+1,teamId:t.id};p.clubCareer[t.id]={apps:0,goals:0,assists:0,seasons:0};t.players.push(p);picks.push({pick:i+1,teamId:t.id,teamName:t.name,playerId:p.id,playerName:p.name,pos:p.pos,ovr:p.ovr})});
 const event={season:world.season.no,picks};world.draftHistory.push(event);world.season.draft=event;world.news.unshift(`🎓 Draft T${world.season.no}: ${picks[0].playerName} fue nº1 para ${picks[0].teamName}.`);return event;
}
...
function renderDraftPanel(){
 const box=$('#draftPanel');if(!box)return;if(!world?.season?.draft){box.style.display='none';return}box.style.display='block';const d=world.season.draft;box.innerHTML=`<div class="draftHead"><div><b>🎓 Draft T${d.season}</b><div class="muted tiny">Orden inverso: último de Segunda recibe el pick nº1; campeón de Primera, el último.</div></div><span class="badge">${d.picks.length} picks</span></div><div class="draftGrid">${d.picks.map(p=>`<div class="draftPick"><span class="pickNo">${p.pick}</span><div><b>${esc(p.playerName)}</b><span>${esc(p.teamName)} · ${p.pos}</span></div><b>${p.ovr}</b></div>`).join('')}</div>`}
function renderMarket(){
...
function statePayload(){if(world)world.rngState=rngState;return{version:'5.1',client:'VM1.1',savedAt:Date.now(),nextTeamId,nextPlayerId,world:world?vm11CompactWorld(world):null,setup:world?null:{teams:structuredClone(teams)}}}
function normalizeWorldV5(w){
 if(!w)return w;(w.teams||[]).forEach(ensureTeamV5);(w.retired||[]).forEach(p=>{p.clubCareer=p.clubCareer||{};p.transferHistory=p.transferHistory||[]});
...
async function vm11CompactExistingStorage(){
 try{
  ...
  if(latest?.payload?.world){const compact={...latest.payload,version:'5.1',client:'VM1.1',world:vm11CompactWorld(normalizeWorldV5(latest.payload.world))};await vmPut('autosave',compact,'Autosave migrado')}
  const manual=all.find(x=>x.slot==='manual');if(manual?.payload?.world){const compact={...manual.payload,version:'5.1',client:'VM1.1',world:vm11CompactWorld(normalizeWorldV5(manual.payload.world))};await vmPut('manual',compact,'Manual migrado')}
  const snaps=all.filter(x=>x.slot.startsWith('snap-')).sort((a,b)=>b.savedAt-a.savedAt);for(const rec of snaps.slice(0,2)){if(rec.payload?.world)await vmPut(rec.slot,{...rec.payload,version:'5.1',client:'VM1.1',world:vm11CompactWorld(normalizeWorldV5(rec.payload.world))},rec.label)}for(const rec of snaps.slice(2))await vm11Delete(rec.slot);
...
function runTests(){
 ...
 try{
  ...
  test('Payload es v5.1 compacto',()=>{resetTestWorld();const p=statePayload();assert(p.version==='5.1'&&p.client==='VM1.1'&&!p.teams,'payload')});
...
$('#runTestsBtn').addEventListener('click',()=>{
  const core=runTests(),extra=vm11BalanceSelfTest(),results=[...core,...extra];
  const passed=results.filter(x=>x.ok).length;
  note(`Tests VM 1.1: ${passed}/${results.length} PASS`);
